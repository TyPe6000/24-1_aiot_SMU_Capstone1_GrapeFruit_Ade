...
const unsigned long Rduration = 4000; // 릴리스 대기 시간 조정
...
void checkStopCondition() {
    // 문이 닫히거나 HDVal이 1로 돌아온 경우
    if ((state.HDVal == 1 || state.MSVal == 0) && !(state.MSVal == 1 && state.HDVal == 1)) {
        state.desiredServoPos = servoIDLE;
        moveServo();
        state.GDCondition = false;

        // ReleaseComplete가 false인 경우에만 Release 동작 시작
    ...
    // 초기 상태로 돌아갔을 때 Release 동작 초기화
    if (state.MSVal == 1 && state.HDVal == 1) {
        state.desiredServoPos = servoIDLE;
        state.GDCondition = false;
        state.Release = false;
        state.ReleaseComplete = false; // 문이 열릴 때 Release 초기화
        moveServo();
    }
}
...
void handleRelease() {
    if (state.Release && !state.ReleaseComplete) {
        ...
        if (elapsedTime <= Rduration) {
            state.desiredServoPos = servoRelease;  // 릴리스 방향으로 모터 작동
        } else {
            state.desiredServoPos = servoIDLE;     // 릴리스 종료 후 모터 정지
            state.Release = false;                // 릴리스 상태 초기화
            state.ReleaseComplete = true;         // 완료 상태 기록
        }
        moveServo();
    }
}
